...
import updateElectionPayments from "../_actions/update-election-payments";
import { PaymentStatus } from "@/types/database";
import { Check, Eye, XCircle } from "lucide-react";

interface ElectionsPaymentsTableProps {
  payments: ElectionPaymentRowData[];
...
export function ElectionsPaymentsTable({
    ...
    return payments.filter((payment) => {
      ...
      if (searchQuery.trim()) {
        ...
        const matchesOrganization = payment.organizations.name
          .toLowerCase()
          .includes(query);
        const matchesOrganizationShorthand = payment.organizations.shorthand_name
          .toLowerCase()
          .includes(query);
        const matchesElection = payment.election_sessions.title
          .toLowerCase()
          .includes(query);
  ...
  const handlePaymentAction = useCallback(
    (paymentId: string, action: "verified" | "rejected") => {
      ...
      startTransition(async () => {
        const result = await updateElectionPayments(paymentId, action);
        if (result.success) {
  ...
  const handleViewReceipt = useCallback((url: string, electionTitle: string) => {
    setSelectedReceipt({ url, electionTitle });
    ...
    <>
        ...
        <div className="border-b border-border bg-muted/20 p-4 md:p-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <PaymentSearchBar
              ...
              placeholder="Search organization, election, contact, or email..."
            />
                  ...
                  <div className="mb-3 flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-foreground">
                        {payment.election_sessions.title}
                      </p>
                      ...
                      <TableCell className="px-4 py-3.5">
                        <div className="flex flex-wrap gap-2">
                          {payment.status === "pending" && (
                            <>
                              ...
                              >
                                {processingPaymentId === payment.id ? (
                                  <>
                                    <span className="mr-2 h-3 w-3 animate-spin rounded-full border-2 border-rose-500 border-t-transparent" />
                                    Rejecting...
                                  </>
                                ) : (
                                  <>
                          ...
                          {payment.status === "verified" && (
                            <span className="self-center text-xs text-emerald-600 dark:text-emerald-400 font-medium">
                              Verified
                            </span>
                          )}
                          {payment.status === "rejected" && (
                            <span className="self-center text-xs text-rose-600 dark:text-rose-400 font-medium">
                              Rejected
                            </span>
                          )}
                        </div>
      ...
      {selectedReceipt && (
        <ReceiptModal
          ...
          title={selectedReceipt.electionTitle}
