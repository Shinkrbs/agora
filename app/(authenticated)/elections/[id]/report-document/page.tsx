...
import { getReportData } from "@/app/(authenticated)/(admin)/election-session-management/election/[id]/reports/_queries/get-report-data";
import { ReportDocument } from "@/app/(authenticated)/(admin)/election-session-management/election/[id]/reports/_components";
  ...
  if (election.status !== "completed") {
    return (
      <div className="min-h-screen bg-muted flex items-center justify-center p-4">
        <Card className="max-w-md w-full p-8 text-center">
          <div className="mb-6 flex justify-center">
            <AlertCircle className="w-16 h-16 text-yellow-600 dark:text-yellow-500" />
          </div>
          <h2 className="text-2xl font-bold text-foreground mb-4">
